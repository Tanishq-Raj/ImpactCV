...
export function TailorCVDialog({ cvData, onTailored }: TailorCVDialogProps) {
  ...
  const [geminiKey, setGeminiKey] = useState(localStorage.getItem('impact-cv-gemini-key'));

  useEffect(() => {
    localStorage.setItem('impact-cv-gemini-key', geminiKey)
  }, [geminiKey]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Using Gemini API key:', geminiKey.substring(0, 5) + '...');
        ...
        contents: [
          {
            ...
            parts: [
              {
                text: `You are a professional CV tailor who specializes in optimizing CVs for specific job applications. 
                ...
                I'm applying for a "${jobTitle}" position. Here's the job description:
